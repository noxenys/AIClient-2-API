...
> このリポジトリには、購入リンク、紹介リンク、コミュニティ導線、QR コードをデフォルトでは含めていません。
>
> 表示したい場合は、このセクションを自身の情報と画像に置き換えてください。
...
> - **2025.12.11** - Docker デプロイ例とローカルイメージビルド手順を追加
...
```bash
docker run -d -p 3000:3000 -p 8085-8086:8085-8086 -p 1455:1455 -p 19876-19880:19876-19880 --restart=always -v "指定パス:/app/configs" --name aiclient2api your-registry/aiclient-2-api:latest
```
...
`your-registry/aiclient-2-api:latest` は自身のイメージアドレスに置き換えてください。プリビルドイメージを持っていない場合は、以下のローカルビルド手順を使用してください。
...
デフォルトの `docker/docker-compose.yml` は現在のリポジトリからローカルビルドする設定です。自身のプリビルドイメージを使いたい場合は、`build:` ブロックを `image: your-registry/aiclient-2-api:latest` に置き換えてから `docker compose up -d` を実行してください。
...
- **イメージを再ビルド**：`docker compose build --no-cache && docker compose up -d`
